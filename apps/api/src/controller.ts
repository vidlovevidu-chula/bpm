...
async function registerBpm(req: Request, res: Response) {
  ...
  console.log(`Hardware ID: ${id} BPM: ${bpm} Registered at ${timestamp.toLocaleString()}`)
...
async function popBpm(req: Request, res: Response) {
  ...
  for (; i < count && bpmRecordsQueue.length > 0; i++) {
    ...
    console.log(
      `Retrieved Hardware ID: ${record.id} BPM: ${record.bpm} at ${record.timestamp.toLocaleString()}`
    )
  }
...
async function getSong(req: Request, res: Response) {
  ...
  const avgBpm = currentBpmRecords.reduce((acc, cur) => acc + cur.bpm, 0) / currentBpmRecords.length
