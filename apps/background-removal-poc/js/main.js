...
async function background_removal(videoTrack) {
  ...
  const transformer = new TransformStream({
    async transform(videoFrame, controller) {
      ...
      detections = faceDetector.detect(videoFrame)
      ...
      if (detections.detections.length == 0) {
        ...
        if (noFaceCount > 15) {
          ...
          // getNextSong(detections.detections.length)
        }
      }
...
document.getElementsByClassName("next-song")[0].addEventListener("click", () => {
  console.log("Click detected")
  getNextSong(detections.detections.length)
})

async function getNextSong(detectionsLength) {
  // call new
